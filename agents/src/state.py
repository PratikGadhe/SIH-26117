...
class WorkbenchState(TypedDict):
    ...
    csv_path: Optional[str]  # Path to uploaded CSV tabular data (if any)
    ...
    vision_data: Optional[
        Dict[str, Any]
    ]  # Results from Member 4 (Qwen3-VL Vision tool)
    ...
    pending_tool_call: Optional[
        Dict[str, Any]
    ]  # Next tool call: {"tool_name": "...", "arguments": {...}}
    tool_results: List[Dict[str, Any]]  # History of executed tool results in this run
    tool_call_count: int  # Count of executed tool calls for loop safety
    max_tool_calls: int  # Maximum permitted tool calls per run
